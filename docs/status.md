...
| Structural/behavioral/mutation coverage | Implemented within named bounds | The retained `strata-txn` evidence records 95.62% lines, 93.55% functions, and 75.44% branches from the nightly LLVM run, plus the 972-mutant campaign and seven remediation slices. Remaining Loom-only/configuration, equivalent, unreachable, and timeout classes are explicitly classified; no universal coverage or mutation threshold is claimed. |
