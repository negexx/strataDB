...
| Snapshot/query reads | Partial | Immutable scan, predicate, explain, and vector-search reads exist. The stable transaction API also provides bounded transaction-base snapshot reads with read-your-writes for lookup, scan (including predicate reads), and group reads; `vector_search` after staged writes returns a typed unsupported-transaction-read error. This is not a general read/write query interface and does not provide full serializability. |
