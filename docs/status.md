...
The current checked-in closeout is based on merged Audit 10 at `c1e2d38` (PR #87);
...
| Performance/resource allocation | Implemented within named bounds | Filtered vector live-set reuse is implemented with bounded per-snapshot caching. The immutable row-ID reservation catalog still has O(reservation-records) recovery reads per allocation attempt, and compaction remains lifecycle-exclusive and whole-dataset materializing; these are documented scalability limits, not universal WAF/RAF/SAF or SLO claims. See [Performance audit](audit/phase-3/txn-sol-performance-resource-audit.md). |
...
| CLI | Implemented within named bounds | Stable administration commands expose `inspect --json`, `schema`, planned `explain --json`, `migration validate/run/status`, `manifest-status`, `recovery-status`, and `evidence`, alongside the existing typed lookup/group-by/query-scan and compatibility commands. Each new administration result has human and JSON rendering; process exit categories are stable as operational `1`, usage `2`, conflict `3`, unsupported `4`, and corruption `5`. `evidence` reports the retained Criterion command rather than treating one CLI invocation as a benchmark. All operations remain local and scoped to the opened dataset; mutation commands, including `migration run`, retain explicit single-writer acknowledgement and none add cross-process coordination. |
| Python | Implemented within named bounds | The versioned PyO3 Dataset/Snapshot/Transaction facade exposes the accepted typed query, migration, planner, and error contracts. In-process binding tests and the retained packaged-wheel import smoke provide implementation/package evidence within the documented local boundary; this is not a release-readiness or unsupported-platform claim. |
...
| Structural/behavioral/mutation coverage | Implemented within named bounds | At remediation head `f851d2b`, retained `strata-txn` evidence records 95.62% lines, 93.55% functions, and 75.44% branches from the nightly LLVM run, plus the 972-mutant campaign and seven remediation slices. Later changes through merged head `d08f5d6` are explicitly outside those measurements; this is not a current-head coverage claim. Remaining Loom-only/configuration, equivalent, unreachable, and timeout classes are explicitly classified; no universal coverage or mutation threshold is claimed. |
| Operational observability | Implemented within named bounds | The shared `Dataset` handle exposes a bounded, redacted in-memory event journal with monotonic per-handle sequence IDs, kind/outcome filtering, snapshot/drain hand-off, and eviction accounting for transaction and lifecycle outcomes. The embedding application owns asynchronous export, retention, and durable transport; this is not a cross-process or compliance audit log. |
| Cryptographic/threat model | Implemented within named bounds | The supported local/shared-handle threat model is documented and guarded by existing path, format, checksum, ownership, range, and bounded-recovery checks, redacted operational events, and crate-level `forbid(unsafe_code)` in `strata-txn` and `strata-storage`. CRC remains accidental-corruption detection rather than authentication; encryption, key management, hostile-filesystem race containment, network/authentication, and tenant security remain explicit non-goals. See [threat model](security/threat-model.md) and [Audit 11 report](audit/phase-3/txn-sol-cryptographic-threat-model-audit.md). |
