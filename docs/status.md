...
| Compaction/GC | Partial | Explicit snapshot-preserving row/segment compaction, post-publication reclamation, age-based manifest retention, recognized-object vacuum, and a maintenance operation whose final inventory may report `storage_bound_met` are implemented. It is not an atomic or continuing bound; active snapshots, protected history, and unsupported object types can keep physical growth above a requested limit. |
