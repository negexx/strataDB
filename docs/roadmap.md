...
| Schema catalog, migrations, point lookup, time travel, stable query API | Phase 2–3 | Versioned schema catalog, the named deterministic `add_nullable_column` migration, point lookup, stable typed query APIs, and administration commands are implemented. Time travel and arbitrary schema evolution remain deferred. |
...
| Usable Python API and stable administration CLI | Phase 2 | Typed Python/CLI query surfaces are implemented and integrated within the documented boundary. The administration CLI exposes the documented inspect/schema/explain/migration/recovery-manifest/evidence commands without adding distributed coordination. |
