...
**Status:** Historical
**Current replacement:** [Phase 3 query-refinement spec](../../design/phase-3-query-refinement-spec.md)
**Archived from:** `docs/design/phase-3-implementation-plan.md`
...
- `Manifest` currently derives `Eq` ÔÇö this task's `Value` enum contains `f64` (`Value::Float64`), and `f64` does not implement `Eq`. `Manifest`'s derive must drop `Eq` (keep `PartialEq`) once `Value` is reachable from it. Nothing in the existing codebase relies on `Manifest: Eq` specifically (only `PartialEq`, via `assert_eq!`) ÔÇö confirmed by grep before this plan was written.

---

### Task 1: `crates/storage` ÔÇö `Value`, `ColumnStats`, `DataFileEntry`, `compute_stats`, manifest schema change
...
//! `docs/design/phase-3-query-refinement-spec.md` ┬º1.
...
/// `strata_query::Predicate` ÔÇö both need the same "which types are
...
Expected: FAIL to compile ÔÇö `compute_stats` is not defined.
...
/// Called on the *original, pre-encoding* batch at commit time ÔÇö see
/// `docs/design/phase-3-query-refinement-spec.md` ┬º1. Columns with
/// no non-null values, or a non-orderable type (e.g. a vector column), get
/// no entry ÔÇö never a wrong or placeholder one.
#[must_use]
...
- `arrow-arith-58.3.0/src/aggregate.rs:928` ÔÇö `pub fn min<T: ArrowNumericType>(array: &PrimitiveArray<T>) -> Option<T::Native>` (and `max` at line 943)
- `arrow-arith-58.3.0/src/aggregate.rs:531,521` ÔÇö `pub fn min_string<T: OffsetSizeTrait>(array: &GenericStringArray<T>) -> Option<&str>` (and `max_string`)
...
Replace the top of the file (imports and struct definitions) ÔÇö old:
...
(Note: `Eq` is dropped from the derive list ÔÇö `Value::Float64(f64)` doesn't implement `Eq`, so it can no longer be derived transitively through `ColumnStats` -> `DataFileEntry` -> `Manifest`. `PartialEq` is kept and is all any existing code needs, per this plan's Global Constraints.)

- [ ] **Step 5: Update `manifest.rs`'s existing tests to the new shape**

Each of the 4 existing tests builds a `Manifest` literal with `data_files: vec!["a.arrow".to_string()]` or similar ÔÇö these no longer compile. Replace every such literal, e.g.:
...
- Produces: `Dataset::data_files(&self) -> &[DataFileEntry]` (changed return type ÔÇö Task 4/5 and the two existing test call sites below depend on this)
...
Expected: FAIL to compile ÔÇö `data_files()[0]` is currently `&String`, which has no `.stats` field, and `entry.stats` doesn't exist yet.
...
### Task 3: `crates/query` ÔÇö `Predicate`, general `filter`, `should_scan_file`
...
Expected: FAIL to compile ÔÇö `filter`/`should_scan_file` not defined.

- [ ] **Step 3: Add `strata-storage` as a dependency**

`crates/query/Cargo.toml` ÔÇö add:
...
/// whenever it can't prove otherwise ÔÇö see
/// `docs/design/phase-3-query-refinement-spec.md` ┬º2. Pure
/// function, zero I/O.
#[must_use]
...
- `arrow-ord-58.3.0/src/cmp.rs:79,113,130,147,164` ÔÇö all five: `pub fn <name>(lhs: &dyn Datum, rhs: &dyn Datum) -> Result<BooleanArray, ArrowError>`
- `arrow-array-58.3.0/src/array/primitive_array.rs:676` ÔÇö `pub fn new_scalar(value: T::Native) -> Scalar<Self>`
...
`crates/query/src/lib.rs` ÔÇö replace the whole file's `filter_eq` implementation and imports:
...
/// convenience wrapper over [`filter`] with [`Predicate::Eq`] ÔÇö kept for
...
(This keeps the existing `filter_eq_keeps_only_matching_rows` test byte-for-byte as a regression guard ÔÇö it must still pass unchanged, proving the wrapper is behaviorally identical to the old direct implementation.)

- [ ] **Step 6: Run tests to verify they pass**

Run: `cargo test -p strata-query`
Expected: all `predicate::tests::*` pass (9 new: 4 filter variants + 5 should_scan_file cases ÔÇö note `should_scan_file_fails_open_on_range_predicates` contains 3 assertions in one test), the existing `filter_eq_keeps_only_matching_rows` still passes unchanged, and all `group_by::tests::*` (8, from Phase 2) are unaffected.

- [ ] **Step 7: Checkpoint**

Run: `cargo clippy -p strata-query --all-targets -- -D warnings && cargo fmt -p strata-query --check`
Expected: clean ÔÇö fix anything flagged (expect at least one `missing_errors_doc` or similar on first pass, matching every prior task's pattern in this codebase). Then `git add crates/query/ && git commit -m "feat(query): add Predicate, general filter, and should_scan_file pruning"`.
...
- Produces: `pub struct ExplainResult { pub total_files: usize, pub scanned: Vec<String>, pub skipped: Vec<String> }`, `Dataset::explain(&self, predicate: &Predicate) -> ExplainResult`, `Dataset::scan_with_predicate(&self, schema: &SchemaRef, predicate: &Predicate) -> Result<RecordBatch>` ÔÇö Task 5's CLI subcommand and integration test consume both.
...
Expected: FAIL to compile ÔÇö `Dataset::explain`/`scan_with_predicate` not defined, and `strata_query` isn't importable from `crates/txn`'s test module (still a dev-dependency only at this point).

- [ ] **Step 3: Promote `strata-query` to a real dependency**

`crates/txn/Cargo.toml` ÔÇö move `strata-query` out of `[dev-dependencies]` into `[dependencies]`:
...
```rust
/// The outcome of [`Dataset::explain`] ÔÇö which files a predicate would
...
```rust
    ...
    /// without opening any file body ÔÇö pure introspection over stats
    /// already loaded in the manifest. See
    /// `docs/design/phase-3-query-refinement-spec.md` ┬º3.
    #[must_use]
    ...
    /// introspection twin ÔÇö both call the exact same
...
Run: `cargo test -p strata-txn -p strata-cli`
Expected: all passing ÔÇö nothing from Phase 1/2 should be affected, since `scan()` and `commit()`'s existing behavior for callers that never touch `explain`/`scan_with_predicate` is unchanged.
...
(Placed as its own arm; exact position among the other arms doesn't matter since `match` doesn't require ordering. Also update the usage line at the top of `run`'s `let Some(cmd) = args.get(1) else { eprintln!("usage: ..."); ... }` block to add `explain` to the listed subcommands ÔÇö the exact string, find the existing `eprintln!("usage: strata <create|insert|scan|filter|search|inspect|crash-loop> ...")` and add `explain` to that list.)
...
Expected output shows `total_files=2`, one file in `scan:`, and (if the two single-row commits' id ranges are disjoint enough ÔÇö they are, `[1,1]` vs `[100,100]`) one file in `skip:`.
...
//! untouched files." See
//! `docs/design/phase-3-query-refinement-spec.md` ┬º3's testing
//! section.
...
Per `CLAUDE.md`'s "What 'done' means" ÔÇö this phase is not complete until the `reviewer` subagent (`.opencode/agents/reviewer.md`, Opus) has reviewed the full diff, the same way Phase 2's whole-branch review caught a Critical `Dataset::scan` regression that no task-scoped review could see (each task-scoped review only ever sees one task's diff). Do not skip this. Do not mark Phase 3 done in conversation before it happens.

Pay particular attention, in that review, to whether `scan_with_predicate`'s pruning and `explain`'s reporting can ever disagree (they must always agree, since both call the exact same `should_scan_file` ÔÇö but verify no future edit could let them drift apart), and whether the manifest's breaking schema change (Task 1) is handled consistently everywhere `Manifest`/`data_files` is touched ÔÇö this plan updated every known call site, but a whole-branch review, not a task-scoped one, is what caught the equivalent gap in Phase 2.
