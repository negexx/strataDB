...
**Status:** Historical
**Current replacement:** [Phase 2 encodings and group-by spec](../../design/phase-2-encodings-and-groupby-spec.md)
**Archived from:** `docs/design/phase-2-implementation-plan.md`
...
**Tech Stack:** Rust, `arrow` (already a workspace dependency ÔÇö no new crates needed; `arrow::row`/`arrow::compute::kernels::cast` are re-exports already available through the existing `arrow` dependency).

## Global Constraints

- Edition 2024, workspace lints apply (`clippy::pedantic` + `clippy::all` at warn) ÔÇö every public `Result`-returning function needs a `# Errors` doc section (see Phase 1's `strata-storage`/`strata-txn` for the established pattern).
- `unwrap()`/`expect()` are `clippy::warn` ÔÇö fine in `#[cfg(test)]` modules (add `#[allow(clippy::unwrap_used, clippy::expect_used)]` on the `mod tests` block, matching every existing test module in this codebase), not fine in library code.
- **Git Flow branching.** Work happens on `feature/phase-2-encodings-and-groupby`, branched from `develop`. Every task's "Checkpoint" step means: run the verification commands, confirm green, then `git add` the task's files and `git commit` with a Conventional Commits message (`feat:`/`test:`/`chore:` ÔÇö see `.opencode/rules/git.md` for the format this project already uses). One commit per task is the floor; commit more often within a task if it helps (e.g. a separate `test:` commit for the failing test before the `feat:` commit that makes it pass) ÔÇö the skill's TDD step structure already implies this rhythm. Never commit directly to `develop` or `main`.
...
- All new public functions in `crates/storage` and `crates/query` return `Result<_, ArrowError>` (or the crate's existing error type for `crates/storage` ÔÇö see Task 1) ÔÇö no new error enum needed for Phase 2.

---

### Task 1: `crates/storage::encoding` ÔÇö dictionary encoding pass
...
**Interfaces:**
- Produces: `pub fn encode_batch(batch: &RecordBatch) -> crate::error::Result<RecordBatch>` ÔÇö Task 2 calls this directly before `write_batch`.
...
//! `docs/design/phase-2-encodings-and-groupby-spec.md` ┬º1.
...
Expected: FAIL to compile ÔÇö `encode_batch` is not defined.
...
`crates/storage/src/lib.rs` ÔÇö add to the existing `pub mod`/`pub use` lists:
...
`crates/storage/Cargo.toml` needs no new entries ÔÇö `arrow::row` and `arrow::compute::cast` are both part of the `arrow` crate already declared as a dependency.
...
Add to `crates/txn/src/dataset.rs`'s existing `#[cfg(test)] mod tests` block (it already has `#[allow(clippy::unwrap_used, clippy::expect_used)]` and the `temp_dir`/`test_schema` helpers ÔÇö reuse them):
...
Then use `ds.data_files()` instead of the placeholder `ds_manifest_data_files(&ds)` helper in the test above ÔÇö replace that line with:
...
Expected: FAIL ÔÇö the written file is still plain `Utf8`, not `Dictionary`, because `commit` doesn't call `encode_batch` yet.

- [ ] **Step 3: Implement ÔÇö call `encode_batch` before `write_batch`**
...
(This replaces the existing `write_batch(&data_dir.join(&file_name), batch)?;` line ÔÇö `batch` becomes `&encoded`.)
...
- [ ] **Step 5: Run the full existing `strata-txn` and `strata-cli` test suites ÔÇö confirm nothing broke**

Run: `cargo test -p strata-txn -p strata-cli`
Expected: every existing test (including `crates/cli/tests/mvp_checklist_6_crash_recovery.rs` and `crates/txn/tests/mvp_checklist_1_to_5.rs`) still passes. Dictionary-encoded columns must round-trip transparently through `Dataset::scan`, `strata_query::filter_eq`, and `strata_index::brute_force_search` ÔÇö if any of those fail, the bug is almost certainly a downstream consumer assuming a concrete non-Dictionary array type instead of going through the generic `Array`/`Datum` interfaces (see the spec's ┬º1 note on why this should already work).
...
### Task 3: `crates/query::group_by` ÔÇö core engine (single group column, `Count`)
...
**Interfaces:**
- Produces: `pub enum AggFunc { Count, Sum, Min, Max, Avg }` and `pub fn group_by(batch: &RecordBatch, group_cols: &[&str], aggs: &[(&str, AggFunc)]) -> Result<RecordBatch, ArrowError>` ÔÇö Task 4 extends this function's body; the signature does not change.
...
//! `docs/design/phase-2-encodings-and-groupby-spec.md` ┬º2.
...
Expected: FAIL to compile ÔÇö `group_by`/`AggFunc` not defined.
...
pub fn group_by(
    ...
    // Count doesn't require a numeric column ÔÇö it only null-checks, so it
...
`RowConverter::convert_rows` (used in `build_result_batch` above) is verified against arrow-row 58.3.0's installed source: `pub fn convert_rows<'a, I>(&self, rows: I) -> Result<Vec<ArrayRef>, ArrowError> where I: IntoIterator<Item = Row<'a>>` ÔÇö matches the `converter.convert_rows(borrowed_keys)?` call exactly, where `borrowed_keys: Vec<Row<'_>>` comes from `owned_keys.iter().map(OwnedRow::row)`.

- [ ] **Step 4: Wire up `lib.rs`**

`crates/query/src/lib.rs` ÔÇö add:
...
Also add the missing `Float64Array` import to `group_by.rs`'s implementation block: `use arrow::array::Float64Array;` (alongside the existing `arrow::array::{Array, ArrayRef, Int64Array, RecordBatch}` import ÔÇö note `Int64Array` is unused by the implementation as written and should be removed from that import list; it was only needed by the test module, which imports it separately).
...
Expected: clean ÔÇö fix any findings before Task 4 (this file is large and pedantic-heavy; expect at least a `missing_errors_doc` or two on first pass, matching the pattern from Phase 1's review).

---

### Task 4: Extend `group_by` ÔÇö multi-column grouping, all `AggFunc` variants, error paths

**Files:**
- Modify: `crates/query/src/group_by.rs` (tests only ÔÇö the implementation from Task 3 already supports multiple group columns and all `AggFunc` variants structurally; this task is primarily test coverage plus the error-path checks the spec requires)

**Interfaces:**
- Consumes/Produces: same signature as Task 3 ÔÇö no API change.
...
```rust
    ...
    fn count_on_non_numeric_column_succeeds() {
        // Count must NOT require a numeric column ÔÇö see Task 3's
...
### Task 5: `bench/` ÔÇö the actual 10M+ row exit-criterion benchmark
...
(Verified current as of this plan's writing: 0.8.2, released 2026-02-04, supports the last three stable Rust minor releases ÔÇö well within this project's MSRV floor. If significant time has passed since this plan was written, re-verify at `https://crates.io/crates/criterion` before trusting this version ÔÇö do not assume it's still current, per this project's own established practice, ADR 0004/0005.)
...
```rust
//! `GROUP BY` benchmark ÔÇö Phase 2's actual exit criterion ("GROUP BY over
//! 10M+ rows, correct, benchmarked"). Correctness against a naive,
//! obviously-right reference implementation is checked *before* the
//! throughput numbers are trusted ÔÇö see
//! `docs/design/phase-2-encodings-and-groupby-spec.md` ┬º3.
...
// data construction ÔÇö allow it here the same way every test module in
...
fn bench_group_by(c: &mut Criterion) {
    ...
    // Correctness gate ÔÇö runs once, before any timed iteration. A fast
...
Expected: `check_correctness` passes silently (a panic there fails the whole `cargo bench` run before any timing output appears ÔÇö that's intentional, it's the correctness gate), followed by Criterion's timing report for `group_by_10m_rows/single_column_sum`. Note the actual throughput number in the task completion report ÔÇö this is the Phase 2 exit criterion's deliverable, not just a passing test.
...
Per `CLAUDE.md`'s "What 'done' means" ÔÇö this phase is not complete until the `reviewer` subagent (`.opencode/agents/reviewer.md`, Opus) has reviewed the full diff, the same way Phase 1's critical fsync-durability bug was only caught by that review, not by the test suite. Do not skip this. Do not mark Phase 2 done in conversation before it happens.
