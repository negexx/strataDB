...
**Row-id / graph label space.** `crates/index` is a from-scratch implementation with no external HNSW library — every row-id is a plain `u64` throughout, with no `usize`-typed foreign API boundary to cross and therefore no separate label-space conversion to reason about. A segment's own internal ordinals (`0..N`, assigned at build time) are mapped back to global row-ids by the segment's own row-id section (`SegmentReader::row_id_at`), entirely inside `crates/index` — `crates/txn` only ever sees real row-ids.
