...
**Visibility is therefore a bound plus an exclusion set**, PostgreSQL's snapshot in miniature (`xmin`/`xmax` plus the `xip_list` of in-progress transactions): a row-id is visible when it is at or below the published bound *and* not among the claims outstanding when that snapshot was published. The exclusion set is sized by the number of concurrently committing transactions — not by dataset size or history length — which is what keeps it affordable on the per-candidate `is_visible` check during HNSW traversal. Abandoned row-ids need no entry: they become permanent gaps, and nothing exists at a gap to be found. Implemented in `crates/txn/src/row_id.rs`; see its module doc.

If the commit fails and the transaction retries per §3 step 5, the row-ids provisionally claimed by the failed attempt are discarded, never reused — a fresh range is claimed on the successful retry. Row-ids are therefore monotonically increasing in successful-commit order but may have gaps; **gaps are safe, reuse is forbidden** (a reused row-id could collide with a still-live row's HNSW graph entry and silently corrupt search results — exactly the failure mode this project's flagship correctness claim exists to rule out). This mirrors how `Manifest.version` already behaves on abort. Per `.claude/rules/concurrency-txn-layer.md`, the counter-advance step needs a `loom` interleaving test proving it is genuinely atomic under concurrent commit attempts — now `dataset::loom_tests::a_published_bound_never_covers_another_transactions_outstanding_claim`, which proves the advance and the in-flight registration cannot be observed torn apart.
