...
**Revised (Phase 6) — where the claim happens.** This section originally required the claim to be part of §3 step 4 (the manifest CAS) itself, "not as a separate operation before or after it." Phase 6's implementation claims *before* the commit lock instead, and this spec now records that as the design rather than as a divergence. The reason is that a row-id is baked into two things written to disk — the data file's `_row_id` column and the delta-log entries — and those files are written and fsynced outside the lock on purpose (§3 step 3 needs no conflict information to proceed). Pulling allocation inside the CAS would pull every data-file fsync in with it, serializing all write I/O through the one choke point.

What the original wording was really protecting is a *visibility* property, not an allocation-order one: §2's "never visible to any other transaction until commit succeeds." Once allocation order can diverge from commit order, a scalar high-water mark cannot express "committed" — `next_row_id` is global, so a watermark derived from it covers row-ids that other, still-in-flight transactions have claimed but not committed, and a reader (which takes no lock) can then observe a row mid-commit. Nor can it be repaired by lowering the mark: clamping it to the lowest outstanding claim would hide the committing transaction's *own* rows, breaking the equally load-bearing rule that an acknowledged write is immediately visible.
