...
//! IS the row-id (`row_id` is the identity function); an immutable segment's
...
//! satisfies this today (it touches no scratch state); the current
//! `SegmentReader` implementation must
//! preserve it too.

/// Traversal-time element access, implemented once per graph representation
/// (a live `Graph<D>` or immutable segment reader).
pub trait NodeSource {
    ...
    /// Whether `local` is tombstoned. Defaults to `false` — an immutable segment
    /// has no per-node deleted flag; deletion there is a manifest-level
    ...
    /// path calls it anymore; the immutable segment reader relies on this
    /// default because deletion is manifest-level there.
